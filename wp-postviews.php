...
Description: Enables you to display how many times a post/page had been viewed. Modified by <a href="http://DPotter.net/Technical/" title="David's Technical Musings">David Potter</a> to include options for when and where to display view counts. <strong>**This plugin has been modified by Crowd Favorite. Review changes before updating it.**</strong>
Version: 1.62
...
function wp_postview_cache_count_enqueue() {
	...
	if (!wp_is_post_revision($post) && (is_single() || is_page())) {
		...
		if ($should_count && defined('WP_CACHE') && WP_CACHE) {
			// TODO: Localize and enqueue script here
			wp_enqueue_script('postviews-cache', plugins_url('postviews-cache.js', __FILE__), array('jquery'));
			wp_localize_script('postviews-cache', 'wpPostViews', array('adminURL' => admin_url('admin-ajax.php', (is_ssl() ? 'https' : 'http')), 'postViewsID' => intval($post->ID)));
		}
	}
}
